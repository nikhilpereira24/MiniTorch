...
def central_difference(f, *vals, arg=0, epsilon=1e-6):
    ...
    """
    vals = list(vals)  # cast vals to a list
    pls = vals.copy()  # make a copy for the plus list
    sls = vals.copy()  # make a copy for the subtract list
    for i, v in enumerate(vals):  # for the index, val in vals
        if i == arg:  # check arg element
            pls[i] = v + epsilon  # update pls i to + eps
            sls[i] = v - epsilon  # update sls i to - eps

    return (f(*pls) - f(*sls)) / (2 * epsilon)  # return central difference
...
class Sigmoid(ScalarFunction):
    ...
    def backward(ctx, d_output):
        ...
        return operators.mul(
            operators.sigmoid(a), (1.0 - operators.sigmoid(a)) * d_output
        )
