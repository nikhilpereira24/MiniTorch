...
class Conv2d(minitorch.Module):
    ...
    def forward(self, input):
        ...
        # print("weight shape", self.weights.value.shape)
...
class Network(minitorch.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = Conv2d(
            1, 4, 3, 3
        )  # applies a convolutions with 8 output channels and a 3x3 kernal
        self.conv2 = Conv2d(
            4, 8, 3, 3
        )  # applies a convolution with 4 output channels and a 3x3 kernel
    ...
    def forward(self, x):
        ...
        x = minitorch.nn.avgpool2d(
            self.out, [4, 4]
        )  # Apply 2D pooling (either Avg or Max) with 4x4 kernel.
        x = x.view(BATCH, 392)
        x = self.linear1.forward(
            x
        ).relu()  # Apply a Linear to size 64 followed by a ReLU
        if self.training:
