...
class Linear(minitorch.Module):
    ...
    def forward(self, inputs):
        # store the intercepts for self from self.bias
        intercepts = [bias.value for bias in self.bias]
        for index, input in enumerate(inputs):
            # iterate through the intercepts index
            for index1 in range(len(self.bias)):
                # update the intercept list with the intercept + the input*weight
                intercepts[index1] = (
                    self.bias[index1].value + input * self.weights[index][index1].value
                )
        return intercepts
