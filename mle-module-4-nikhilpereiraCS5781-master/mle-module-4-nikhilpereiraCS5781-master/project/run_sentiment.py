...
class Conv1d(minitorch.Module):
    ...
    def forward(self, input):
        ...
        # print("weights shape", self.weights.value.permute(0,2,1).shape)
...
class CNNSentimentKim(minitorch.Module):
    ...
    def __init__(
        ...
        self.conv1 = Conv1d(
            embedding_size, feature_map_size, filter_sizes[0]
        )  # first convolution layer
        self.conv2 = Conv1d(
            embedding_size, feature_map_size, filter_sizes[1]
        )  # second convolution layer
        self.conv3 = Conv1d(
            embedding_size, feature_map_size, filter_sizes[2]
    ...
    def forward(self, embeddings):
        ...
        embeddings = embeddings.permute(
            0, 2, 1
        )  # new embedding shape batch x embedding dim (50) x sentence length 52

        x1 = self.conv1.forward(embeddings).relu()  # apply relus to convolution layer 1
        x2 = self.conv2.forward(embeddings).relu()  # apply relus to convolution layer 2
        x3 = self.conv3.forward(embeddings).relu()  # apply relus to convolution layer 3
        x_mid = (
            minitorch.nn.max(x1, 2) + minitorch.nn.max(x2, 2) + minitorch.nn.max(x3, 2)
        )
        x = self.final(
            x_mid.view(x_mid.shape[0], x_mid.shape[1])
        )  # Apply a linear to size C = 1 (0, 1) number of classes, followed by a relu
        if self.training:
            x = minitorch.nn.dropout(
                x, self.dropout
            )  # Apply a 25% dropout if the model is in training mode
        return x.sigmoid().view(
            embeddings.shape[0]
        )  # Apply a sigmoid over the class dimension (10,)
